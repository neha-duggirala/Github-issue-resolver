...
def get_pr_info_from_url(pr_url):
    ...
    try:
        ...
        if len(path_parts) >= 4 and path_parts[2] == "pull" and path_parts[3].isdigit():
            ...
            print(
                f"Warning: Could not parse URL path '{parsed_url.path}'. Using defaults."
            )
...
schema_get_pr_info = types.FunctionDeclaration(
    name="get_pr_info_from_url",
    description="Parses a GitHub Pull Request URL to extract OWNER, REPO, PULL_NUMBER",
